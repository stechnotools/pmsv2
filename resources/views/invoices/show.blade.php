...
(isset($paymentSetting['is_yookassa_enabled']) && $paymentSetting['is_yookassa_enabled'] == 'on'))
    ...
    @auth('web')
        @if ($currentWorkspace->creater->id == Auth::user()->id)
            ...
            <a href="#" class="btn btn-sm  mx-1 btn-primary cp_link " data-bs-toggle="tooltip"
                data-bs-original-title="{{ __('Copy Invoice') }}"
                data-link="{{ route('pay.invoice', [$currentWorkspace->slug, \Illuminate\Support\Facades\Crypt::encrypt($invoice->id)]) }}"
                data-toggle="tooltip" data-original-title="{{ __('Click to copy invoice link') }}"><span
                                ...
                                <tbody>
                                    @foreach ($invoice->items as $key => $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->task ? $item->task->title : '' }}-
                                                <b>{{ $item->task ? $item->task->invoiceproject->name : '' }}</b>
                                            </td>
                                            <td>{{ $currentWorkspace->priceFormat($item->price * $item->qty) }}</td>
                                            @auth('web')
                                                <td class="text-right">
...
(isset($paymentSetting['is_yookassa_enabled']) && $paymentSetting['is_yookassa_enabled'] == 'on'))
                                    <ul class="nav nav-tabs bordar_styless py-3">
                                        ...
                                        <li class="pt-3">
                                           <a data-toggle="tab" href="#midtrans-payment" role="tab" aria-controls="midtrans"
                                       ...
                                       @if (isset($paymentSetting['is_xendit_enabled']) && $paymentSetting['is_xendit_enabled'] == 'on')
                                        <li class="pt-3">
                                           <a data-toggle="tab" href="#xendit-payment" role="tab" aria-controls="xendit"
                                       ...
                                       @if (isset($paymentSetting['is_yookassa_enabled']) && $paymentSetting['is_yookassa_enabled'] == 'on')
                                        <li class="pt-3">
                                            <a data-toggle="tab" href="#yookassa-payment" role="tab" aria-controls="yookassa"
                                        ...
                                        <form method="post"
                                            ...
                                            <div class="row">
                                                <div class="form-group col-md-12">
                                                    ...
                                                    <div class="form-icon-user">
                                                        <span class="currency-icon bg-primary">{{
                                                            !empty($currentWorkspace->currency) ? $currentWorkspace->currency : '$'
                                                            }}</span>
                                                        <input class="form-control currency_input" required="required" min="0"
                                                            name="amount" type="number"
                                                            value="{{ $invoice->getDueAmounts($invoice->id) }}" min="0" step="0.01"
                                                            max="{{ $invoice->getDueAmounts($invoice->id) }}" id="amount">
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-md-12 modal-footer">
